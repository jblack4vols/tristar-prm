...
export default function DashboardPage() {
  ...
  const kpis = useMemo(() => {
    const src = filtered;
    if (!src.length) return null as any;
    const total = src.length;
    const uniqueDocs = new Set(src.map((r) => (r.referring_doctor || "").trim()).filter(Boolean)).size;
    const pt = src.filter((r) => (r.discipline || "").toUpperCase() === "PT").length;
    const ot = src.filter((r) => (r.discipline || "").toUpperCase() === "OT").length;

    const dates = src.map((r) => new Date(r.created_date as any)).filter((d) => !isNaN(d as any));
    ...
    const scheduled = src.filter(r => !!r.first_scheduled_date).length;
    const arrived = src.filter(r => !!r.first_arrived_date).length;
    const scheduledPct = total ? (scheduled / total * 100).toFixed(1) : "0.0";
    const arrivedPct = total ? (arrived / total * 100).toFixed(1) : "0.0";

    const today = new Date();
    const noSchedule7d = src.filter(r => {
      const created = r.created_date ? new Date(r.created_date) : null;
      const hasSched = !!r.first_scheduled_date;
      if (!created || hasSched) return false;
      const diffDays = Math.floor((Number(today) - Number(created)) / (1000*60*60*24));
      return diffDays >= 7;
    }).length;
    const noArrival14d = src.filter(r => {
      const sched = r.first_scheduled_date ? new Date(r.first_scheduled_date) : null;
      const hasArr = !!r.first_arrived_date;
      if (!sched || hasArr) return false;
      const diffDays = Math.floor((Number(today) - Number(sched)) / (1000*60*60*24));
      return diffDays >= 14;
    }).length;
    ...
    return {
      ...
      ot,
      perDay,
      span: min && max ? `${asISO(min.toISOString())} → ${asISO(max.toISOString())}` : "-",
      scheduled,
      arrived,
      scheduledPct,
      arrivedPct,
      ...
      {loading ? (
        ...
        <div className="space-y-6">
          <div className="grid grid-cols-2 md:grid-cols-6 lg:grid-cols-8 gap-3">
            ...
            <div className="rounded-2xl border bg-white p-3"><div className="text-xs text-zinc-500">Unique Doctors</div><div className="text-2xl font-semibold">{kpis.uniqueDocs}</div></div>
            ...
            <div className="rounded-2xl border bg-white p-3"><div className="text-xs text-zinc-500">Exceptions</div><div className="text-sm"><span className="font-semibold">{kpis.noSchedule7d}</span> no schedule ≥7d · <span className="font-semibold">{kpis.noArrival14d}</span> no arrival ≥14d</div></div>
          </div>
